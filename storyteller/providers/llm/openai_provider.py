    ...
    async def generate_story_stream(
        ...
        try:
            ...
            error = ProviderError(
                ...
                message=f"OpenAI API request failed: {e.__class__.__name__}",
                is_recoverable=True
            )
            self.set_status(ProviderStatus.ERROR, error)
            raise error from e
            ...
            error = ProviderError(
                ...
                message=f"Failed to decode OpenAI API response: {e}",
                is_recoverable=True
            )
            self.set_status(ProviderStatus.ERROR, error)
            raise error from e
            ...
            error = ProviderError(
                provider_name=self.name,
                error_type="unknown",
                message=f"An unexpected error occurred in OpenAI provider: {str(e)}",
